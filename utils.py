...
def publish_with_retry(formulas, db, queue, retries=3, base_delay=1.0, max_delay=10.0):
    ...
    for attempt in range(retries):
        try:
            ...
            #queue.remove(formulas)  # theoretically we should never get here because if both the db and the queue were both updated, it should have been a success. But to cover all cases, we do handle cleaning up the queue as part of the rollback strategy.

            # Assess remaining attempts:
            if attempt == retries - 1:
                raise # when final attempt has failed
